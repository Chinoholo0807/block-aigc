...
class Task:
    ...
    def vector(self):
        ...
        vec = np.hstack([self.norm_d, self.norm_c,
                        self.norm_runtime, self.norm_n_max])
        return vec
...
class TaskGenerator:
    ...
    def __next__(self):
        ...
        required_d = np.random.choice(
            SINGLE_D_RANGE, size=NUM_D_TYPES)  # 任务数据分布
        ...
        task = Task(task_id, arrival_time, required_d, required_c,
                    required_r_local, required_r_global, required_n_max)
